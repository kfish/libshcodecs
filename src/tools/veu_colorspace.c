...
static int locate_sh_veu_uio_device(char *name, struct sh_veu_uio_device *udp)
{
...
static int setup_uio_map(struct sh_veu_uio_device *udp, int nr, struct uio_map *ump)
{
	char fname[MAXNAMELEN], buf[MAXNAMELEN];
...
#endif
	uv = (unsigned char *)sh_veu_uio_mem.iomem + info->offsets[frame] + UVOffs;
			  ...
			  struct sh_veu_plane *dst)
{
	...
	write_reg(&sh_veu_uio_mmio, src->width | (src->height << 16), VESSR);
	...
	if (do_rgb) {
		...
		write_reg(&sh_veu_uio_mmio, (6 << 16) | (0 << 14) | 2 | 4, VTRCR);
			 ...
			 struct sh_veu_plane *tmp)
{
	...
	while (dst->width == 800 && dst->height == 480) {
		if (!strcmp(envstr, "buf=220x140")) {
			tmp->width = 220; tmp->height = 140;
			break;
		}

		if (!strcmp(envstr, "buf=256x240")) {
			tmp->width = 256; tmp->height = 240;
			dst->width = 800; dst->height = 472;
			break;
		}

		if (!strcmp(envstr, "buf=440x280")) {
			tmp->width = 440; tmp->height = 280;
			break;
		}

		if (!strcmp(envstr, "buf=512x480")) {
			tmp->width = 512; tmp->height = 480;
			dst->width = 800; dst->height = 472;
			break;
		}
		break;
			...
			unsigned long width, unsigned long height)
{
	...
	/* Enable interrupt in UIO driver */
	{
		...
		if ((ret = write(sh_veu_uio_dev.fd, &enable, sizeof(u_long))) != (sizeof(u_long))) {
...
static int sh_veu_config_playback(vidix_playback_t *info)
{
	...
	switch(info->fourcc)
	{
			...
			VOffs=info->offset.v = info->offset.u + (uv_pitch*src_h/2);
