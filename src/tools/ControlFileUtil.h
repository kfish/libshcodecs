...
extern "C" {
	...
	int GetFromCtrlFTop(const char *control_filepath, ENC_EXEC_INFO * enc_exec_info, long *stream_type);	/* 041201 */
	...
	int GetFromCtrlFtoEncExecInfo(FILE * fp_in,
				      ENC_EXEC_INFO * enc_exec_info);
	...
	int ReadUntilKeyMatch(FILE * fp_in, const char *key_word,
			      char *buf_value);
