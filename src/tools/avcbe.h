...
*                                                                            *
*    Version¡¡2.1 :   avcbe.h                                                *
*                                 2006/03/08 17:05 Renesas Technology Corp.  *
*                                                                            *
*****************************************************************************/
...
long avcbe_set_default_param(long stream_type, unsigned long ratecontrol_mode, 
							 avcbe_encoding_property *param, void *other_options);

typedef void (*avcbe_buf_continue_userproc_ptr) (avcbe_stream_info *context, unsigned char *previous_stream_buff,
											long output_size, unsigned char **next_stream_buff, long *stream_buff_size);

long avcbe_init_encode(avcbe_encoding_property *param, avcbe_encoding_property *paramR,
						void *other_options,
						avcbe_buf_continue_userproc_ptr avcbe_buf_continue_userproc,
						TAVCBE_WORKAREA *workarea_info,
						TAVCBE_WORKAREA *dp_workarea_info,
						avcbe_stream_info **context);

long avcbe_init_memory(avcbe_stream_info *context, unsigned long nrefframe,
						unsigned long nldecfmem, TAVCBE_FMEM ldecfmemp[], long ldecwx, long ldecwy);

long avcbe_set_image_pointer(avcbe_stream_info *context, TAVCBE_FMEM *captfmemp, unsigned long ldec,
						unsigned long ref1, unsigned long ref2);

long avcbe_encode_picture(avcbe_stream_info *context, long frm, long set_intra, long output_type,
										TAVCBE_STREAM_BUFF *stream_buff, TAVCBE_STREAM_BUFF *extra_stream_buff);

long avcbe_put_end_code(avcbe_stream_info *context, TAVCBE_STREAM_BUFF *stream_buff,
						unsigned long output_type);

long avcbe_put_filler_data(TAVCBE_STREAM_BUFF *stream_buff, long out_start_code, long output_size);

long avcbe_set_weightedQ(avcbe_stream_info *context, long weightedQ_enable, void *weightedQ);

long avcbe_get_backup(avcbe_stream_info *context, TAVCBE_WORKAREA *backup);

long avcbe_set_backup(avcbe_stream_info *context, TAVCBE_WORKAREA *backup);

long avcbe_get_last_frame_stat(avcbe_stream_info *context, avcbe_frame_stat *fstat);

long avcbe_get_frame_size(avcbe_stream_info *context, long *xs, long *ys);

unsigned long avcbe_get_version(void);

long avcbe_get_last_slice_stat(avcbe_stream_info *context, avcbe_slice_stat *slice_stat);

long avcbe_get_header_information(avcbe_stream_info *context, void  *header_stat, long output_type);

long avcbe_set_VUI_parameters(avcbe_stream_info *context, avcbe_vui_main_param *vui_param);

long avcbe_put_SEI_parameters(avcbe_stream_info *context, long sei_type, void *sei_param,
							TAVCBE_STREAM_BUFF *stream_buff);

long avcbe_change_enc_param(avcbe_stream_info *context, long operation, avcbe_property_after_change *param_after_change);

long avcbe_clear_user_data(avcbe_stream_info *context, long area_mode);

long avcbe_set_user_data(avcbe_stream_info *context, unsigned char *usrdata, long area_mode);

long avcbe_set_gov_time(avcbe_stream_info *context, long tH, long tM, long tS);

long avcbe_get_buffer_check(avcbe_stream_info *context, AVCBE_FRAME_CHECK *parray);

long avcbe_set_quant_type1(avcbe_stream_info *context, avcbe_quant_type1_matrix *quant_matrix);

long avcbe_get_cpb_buffer_size(avcbe_stream_info *context);

long avcbe_calc_cpb_buff_offset(long bitrate, long max_cpb_buff_size, long rate);

long avcbe_calculate_initQ(long stream_type, long bitrate, long xpic_size, long ypic_size, long frame_rate);
