...
{  
	...
	/*--- The MPEG-4&H.264 Encoder Library API (common to MPEG-4&H.264 Decoder) (required-1)@initialize VPU4 ---*/
	/* needs be called only once */
	return_code = m4iph_vpu4_init(&(appli_info->vpu4_param));
	if (return_code < 0) {	/* error */ 
		if (return_code == -1) {
			DisplayMessage(" encode_1file_h264:m4iph_vpu4_init PARAMETER ERROR! ", 1);  
		}    
		appli_info->error_return_function = -101;
	...
	return_code = init_for_encoder_h264(case_no, appli_info, stream_type, &my_context);
	if(return_code != 0){
		return (-114);
	}
	...
	if (return_code <= 0) {
		if (return_code == -4) {
			DisplayMessage(" encode_1file_h264:avcbe_close_encode OUTPUT BUFFER SIZE SHORT ERROR! ", 1);  
		}
	...
	if (appli_info->output_filler_enable == 1) {
		return_code = avcbe_put_filler_data(&my_stream_buff_info, appli_info->other_options_h264.avcbe_put_start_code, 2);
	}
	return (0);
}
...
long init_for_encoder_h264(long case_no, APPLI_INFO *appli_info, long stream_type, avcbe_stream_info **context)
{
	...
	return_code = avcbe_set_default_param(stream_type, AVCBE_RATE_NORMAL, &(appli_info->param),
											 (void *)&(appli_info->other_options_h264));
	if (return_code != 0) {	/* error */
		DisplayMessage(" encode_1file_h264:avcbe_set_default_param ERROR! ", 1);
	...
	if (return_code == -1) {	/* error */
		DisplayMessage(" encode_1file_h264:select_inputfile_set_param ERROR! ", 1);
	...
	if (appli_info->other_options_h264.avcbe_ratecontrol_cpb_buffer_mode == AVCBE_MANUAL) {
		appli_info->other_options_h264.avcbe_ratecontrol_cpb_offset =
			 (unsigned long)avcbe_calc_cpb_buff_offset(appli_info->param.avcbe_bitrate,
			(appli_info->other_options_h264.avcbe_ratecontrol_cpb_max_size * 
			appli_info->other_options_h264.avcbe_ratecontrol_cpb_buffer_unit_size),90);
	}
	return_code = avcbe_init_encode(&(appli_info->param), &(appli_info->paramR), &(appli_info->other_options_h264),
			(avcbe_buf_continue_userproc_ptr)NULL, &WORK_ARRY[0], &WORK_ARRY[1], context);
	if (return_code < 0) {	/* error */ 
		if (return_code == -1) {
			DisplayMessage(" encode_1file_h264:avcbe_init_encode PARAMETER ERROR! ", 1);  
		} else if (return_code == -2) {
			DisplayMessage(" encode_1file_h264:avcbe_init_encode PARAMETER CHANGED! (Maybe Error) ", 1);  
		} else if (return_code == -3) {
			DisplayMessage(" encode_1file_h264:avcbe_init_encode SEQUENCE ERROR! ", 1);
		} else if (return_code == -4) {
			DisplayMessage(" encode_1file_h264:avcbe_init_encode WORK-AREA SIZE SHORT ERROR! ", 1);
		}
	...
	if (return_code != 0) {
		if (return_code == -1) {
			DisplayMessage(" encode_1file_h264:avcbe_init_memory PARAMETER ERROR! ", 1);  
		} else if (return_code == -3) {
			DisplayMessage(" encode_1file_h264:avcbe_init_memory SEQUENCE ERROR! ", 1);
		} else if (return_code == -4) {
			DisplayMessage(" encode_1file_h264:avcbe_init_encode WORK-AREA SIZE SHORT ERROR! ", 1);
		}
...
long encode_picture_unit(long case_no, APPLI_INFO *appli_info, long stream_type, avcbe_stream_info *context)
{
	...
	if (appli_info->other_options_h264.avcbe_out_vui_parameters == AVCBE_ON) { /* output VUI parameters */
		...
		if (return_code <= 0) {
			...
			if (appli_info->other_API_enc_param.vui_main_param.avcbe_nal_hrd_parameters_present_flag == AVCBE_ON) {
				appli_info->other_API_enc_param.vui_main_param.avcbe_nal_hrd_param.avcbe_cpb_size_scale = return_code;
				
			} else if (appli_info->other_API_enc_param.vui_main_param.avcbe_vcl_hrd_parameters_present_flag == AVCBE_ON) {
				appli_info->other_API_enc_param.vui_main_param.avcbe_vcl_hrd_param.avcbe_cpb_size_scale = return_code;
			}
		}		
		appli_info->other_API_enc_param.vui_main_param.avcbe_video_signal_type_present_flag = AVCBE_OFF;//@061215 AVCBE_ON;
		return_code = avcbe_set_VUI_parameters(context, &(appli_info->other_API_enc_param.vui_main_param));
		if (return_code != 0) {
	...
	my_extra_stream_buff_info.buff_top = (unsigned char *)&my_extra_stream_buff[0];
	my_extra_stream_buff_info.buff_size = 16;
	extra_stream_buff = &my_extra_stream_buff_info;
	/* SPS-output-buffer */
	my_sps_stream_buff_info.buff_top = ALIGN(&my_sps_stream_buff[0], 32);
	my_sps_stream_buff_info.buff_size = MY_SPS_STREAM_BUFF_SIZE;

	/* PPS-output-buffer */
	my_pps_stream_buff_info.buff_top = ALIGN(&my_pps_stream_buff[0], 32);
	my_pps_stream_buff_info.buff_size = MY_PPS_STREAM_BUFF_SIZE;

	/* SEI-output-buffer */
	my_sei_stream_buff_info.buff_top = (unsigned char *)&my_sei_stream_buff[0];
	my_sei_stream_buff_info.buff_size = MY_SEI_STREAM_BUFF_SIZE;

	/* Filler Data-output-buffer */
	my_filler_data_buff_info.buff_top  = (unsigned char *)&my_filler_data_buff[0];
	...
	while (1) {	/*--------------------- Repeating by frame numbers --------------------------*/
		...
		if (header_output_flag == 1) {
			...
			if (return_code == AVCBE_SPS_OUTPUTTED) { /* 6 */
				...
				sprintf(messeage_buf, "%s:%d: ERROR OUTPUT SEQUENCE PARAMETER SET return_code=%d ", __FUNCTION__, __LINE__, (int)return_code);
				DisplayMessage(messeage_buf, 1);
			...
			if (return_code == AVCBE_PPS_OUTPUTTED) { /* 7 */
				...
				sprintf(messeage_buf, " ERROR OUTPUT PICTURE PARAMETER SET return_code=%d ", (int)return_code);
				DisplayMessage(messeage_buf, 1);
				appli_info->error_return_function = -111;
				appli_info->error_return_code = return_code;
				return (-111);
		...
		if (return_code != 0) {
			if (return_code == -1) {
				DisplayMessage(" encode_1file_h264:avcbe_set_image_pointer PARAMETER ERROR! ", 1);  
			}
...
//		printf("enc_pic0=%ld,",tv.tv_usec);
		gettimeofday(&tv, &tz);
		return_code = avcbe_encode_picture(context, frm, appli_info->set_intra, appli_info->output_type, &my_stream_buff_info, extra_stream_buff);
		gettimeofday(&tv1, &tz);
...
//		printf("Total sleep  time = %d(msec)\n",m4iph_sleep_time_get());
		if (return_code < 0) {	/* error */ 
			if (return_code == AVCBE_ENCODE_ERROR) { /* -1 */
				...
				DisplayMessage(" encode_1file_h264:avcbe_encode_picture PARAMETER ERROR! ", 1);

			} else if (return_code == AVCBE_NOT_IN_ORDER_ERROR) { /* -3 */
				DisplayMessage(" encode_1file_h264:avcbe_encode_picture SEQUENCE ERROR! ", 1);

			} else if (return_code == AVCBE_OUTPUT_BUFFER_SHORT_ERROR) { /* -4 */
				DisplayMessage(" encode_1file_h264:avcbe_encode_picture OUTPUT BUFFER SIZE SHORT ERROR! ", 1);

			} else if (return_code == AVCBE_VPU_ERROR_AFTER_ENCODING) { /* -6 */
				DisplayMessage(" encode_1file_h264:avcbe_encode_picture VPU4 ERROR! ", 1);

			} else if (return_code == AVCBE_TEMPORARY_BUFFER_SHORT_ERROR) { /* -8 */
				DisplayMessage(" encode_1file_h264:avcbe_encode_picture TEMPORARY_BUFFER SIZE SHORT ERROR! ", 1);

			} else if (return_code == AVCBE_VUI_PARAMETERS_NOT_SPECIFIED_ERROR) { /* -9 */
				DisplayMessage(" encode_1file_h264:avcbe_encode_picture VUI_PARAMETERS NOT SPECIFIED ERROR! ", 1);

			} else if (return_code == AVCBE_WORK_AREA_SHORT_ERROR) { /* -99 */
				DisplayMessage(" encode_1file_h264:avcbe_encode_picture OTHER WORK SIZE SHORT ERROR! ", 1);
...
//			DisplayMessage(messeage_buf, 1);
			...
			sprintf(messeage_buf, " encode_1file_h264:avcbe_encode_picture THIS FRAME SKIPPED(Not Encoded)  frm=%d,seq_no=%d ",
						 (int)frm, (int)appli_info->frame_counter);
			DisplayMessage(messeage_buf, 1);
			...
			sprintf(messeage_buf, " encode_1file_h264:avcbe_encode_picture YET 1 PICTURE NOT FINISHED frm=%d,seq_no=%d ",
						 (int)frm, (int)appli_info->frame_counter);
			DisplayMessage(messeage_buf, 1);
		} else {
			sprintf(messeage_buf, " encode_1file_h264:avcbe_encode_picture UNKNOWN RETURN CODE=%d  frm=%d,seq_no=%d ",
						 (int)return_code, (int)frm, (int)appli_info->frame_counter);
			DisplayMessage(messeage_buf, 1); 
		}
		/* get the information about the just encoded frame (slice) */
		avcbe_get_last_slice_stat(context, &slice_stat);
		appli_info->slice_mb_counter = slice_stat.avcbe_encoded_MB_num;
		appli_info->mb_num_of_picture = slice_stat.avcbe_total_MB_in_frame;
		if (return_code == AVCBE_ENCODE_SUCCESS) {	/* If the return value of avcbe_encode_picture function is 1 */
			...
			/* output AU delimiter */
			if (extra_stream_buff != NULL) {
				cat_output_stream(appli_info, context, (char *)&my_extra_stream_buff[0], slice_stat.avcbe_AU_unit_bytes);
				fwrite((char *)&my_extra_stream_buff[0], slice_stat.avcbe_AU_unit_bytes, 1, appli_info->output_file_fp); 
			}

			/* If the type is IDR-picture, output SPS and PPS data (for 2nd frame and later) */
			if ((header_output_flag == 0)&&(slice_stat.avcbe_encoded_pic_type == AVCBE_IDR_PIC)) {
				...
				if (return_code == AVCBE_SPS_OUTPUTTED) { /* 6 */
					...
					cat_output_stream(appli_info, context, (char *)my_sps_stream_buff_info.buff_top, slice_stat.avcbe_SPS_unit_bytes); 
					fwrite((char *)my_sps_stream_buff_info.buff_top, 1, slice_stat.avcbe_SPS_unit_bytes, appli_info->output_file_fp);
				} else {
					sprintf(messeage_buf, " ERROR OUTPUT SEQUENCE PARAMETER SET return_code=%d ", (int)return_code);
					DisplayMessage(messeage_buf, 1);
					appli_info->error_return_function = -111;
					appli_info->error_return_code = return_code;
					return (-111);
				...
				if (return_code == AVCBE_PPS_OUTPUTTED) { /* 7 */
					...
					cat_output_stream(appli_info, context, (char *)my_pps_stream_buff_info.buff_top, slice_stat.avcbe_PPS_unit_bytes); 
					fwrite((char *)my_pps_stream_buff_info.buff_top, 1,slice_stat.avcbe_PPS_unit_bytes, appli_info->output_file_fp); 
				} else {
					sprintf(messeage_buf, " ERROR OUTPUT PICTURE PARAMETER SET return_code=%d ", (int)return_code);
					DisplayMessage(messeage_buf, 1);
					appli_info->error_return_function = -111;
				...
				/* concatenate the SPS data */
				cat_output_stream(appli_info, context, (char *)&my_sps_stream_buff[0], slice_stat.avcbe_SPS_unit_bytes); 
				fwrite((char *)my_sps_stream_buff_info.buff_top, slice_stat.avcbe_SPS_unit_bytes, 1, appli_info->output_file_fp);
				/* concatenate the PPS data */
				cat_output_stream(appli_info, context, (char *)&my_pps_stream_buff[0], slice_stat.avcbe_PPS_unit_bytes); 
				fwrite((char *)my_pps_stream_buff_info.buff_top, slice_stat.avcbe_PPS_unit_bytes, 1, appli_info->output_file_fp); 
			...
			if ((appli_info->output_filler_enable == 1)&&(appli_info->output_filler_data > 0)) {
				cat_output_stream(appli_info, context, (char *)&my_filler_data_buff[0], appli_info->output_filler_data);
				fwrite((char *)&my_filler_data_buff[0], 
					appli_info->output_filler_data,
					1, appli_info->output_file_fp);
			}
...
long encode_nal_unit(long case_no, APPLI_INFO *appli_info, long stream_type, avcbe_stream_info *context)
{
    ...
    while (1) {	/*--------------------- Repeating by frame numbers --------------------------*/
		...
		/*--- The MPEG-4 Encoder Library API (required-8)@encode each screen of display data ---*/
		return_code = avcbe_encode_picture(context, frm, appli_info->set_intra, appli_info->output_type, 
										&my_stream_buff_info, extra_stream_buff);
		gettimeofday(&tv1, &tz);
		printf("ret=%d,enc_pic1=%ld,",return_code,tv1.tv_usec);
		tm = (tv1.tv_usec-tv.tv_usec)/1000;
		if ( tm<0 ) {
		...
		if (return_code == AVCBE_SLICE_REMAIN) { /* 5 */
			...
			if((appli_info->other_options_h264.avcbe_use_slice == AVCBE_ON) &&
				(appli_info->other_options_h264.avcbe_call_unit == AVCBE_CALL_PER_NAL)){  /* when the avcbe_encode_picture function returns on each 1 slice */
			...
			if ((header_output_flag == 0)&&(slice_stat.avcbe_encoded_pic_type == AVCBE_IDR_PIC)) {
				...
				if (return_code == AVCBE_SPS_OUTPUTTED) { /* 6 */
					...
					sprintf(messeage_buf, "%s: ERROR OUTPUT SEQUENCE PARAMETER SET return_code=%d ", __FUNCTION__, (int)return_code);
					DisplayMessage(messeage_buf, 1);
					appli_info->error_return_function = -111;
					appli_info->error_return_code = return_code;
					return (-111);
			...
			if ((appli_info->output_filler_enable == 1)&&(appli_info->output_filler_data > 0)) {
				cat_output_stream(appli_info, context, (char *)&my_filler_data_buff[0], appli_info->output_filler_data);
				fwrite((char *)&my_filler_data_buff[0], appli_info->output_filler_data, 1, appli_info->output_file_fp);
			}
			...
			dummy_nal_buf_addr += tmp_slice_size;	/* update the address of buffer for 1-slice */
			/* concatenate slice header and slice data */
			cat_output_stream(appli_info, context, dummy_nal_buf, tmp_pic_total_bytes);
...
/*------------------------------------------------------------------*/
long output_SEI_parameters(APPLI_INFO *appli_info, avcbe_stream_info *context,TAVCBE_STREAM_BUFF *sei_stream_buff_info)
{
	long return_code;
	long return_value=0;

	/* output SEI parameter (step1) */
	if (appli_info->other_API_enc_param.out_buffering_period_SEI == AVCBE_ON) {
		return_code = avcbe_put_SEI_parameters(context, AVCBE_SEI_MESSAGE_BUFFERING_PERIOD,
			 (void *)&(appli_info->other_API_enc_param.sei_buffering_period_param),	sei_stream_buff_info);
		/* concatenate SEI parameter (return_code means the size of SEI parameter in byte unit) */
		if (return_code > 0) {
			cat_output_stream(appli_info, context, (char *)sei_stream_buff_info->buff_top, return_code);
	...
	if (appli_info->other_API_enc_param.out_pic_timing_SEI == AVCBE_ON) {
		return_code = avcbe_put_SEI_parameters(context, AVCBE_SEI_MESSAGE_PIC_TIMING, (void *)&(appli_info->other_API_enc_param.sei_pic_timing_param),
	...
	if (appli_info->other_API_enc_param.out_pan_scan_rect_SEI == AVCBE_ON) {
		return_code = avcbe_put_SEI_parameters(context, AVCBE_SEI_MESSAGE_PAN_SCAN_RECT,
				 (void *)&(appli_info->other_API_enc_param.sei_pan_scan_rect_param), sei_stream_buff_info);
		/* concatenate SEI parameter (return_code means the size of SEI parameter in byte unit) */
		if (return_code > 0) {
			cat_output_stream(appli_info, context, (char *)sei_stream_buff_info->buff_top, return_code);
			fwrite((char *)sei_stream_buff_info->buff_top, return_code, 1, appli_info->output_file_fp);
			return_value = 0;
		} else { /* error */
			return_value |= 0x4;
		}
	...
	if (appli_info->other_API_enc_param.out_filler_payload_SEI == AVCBE_ON) {
		return_code = avcbe_put_SEI_parameters(context, AVCBE_SEI_MESSAGE_FILLER_PAYLOAD,
				 (void *)&(appli_info->other_API_enc_param.sei_filler_payload_param), sei_stream_buff_info);
		/* concatenate SEI parameter (return_code means the size of SEI parameter in byte unit) */
		if (return_code > 0) {
			cat_output_stream(appli_info, context, (char *)sei_stream_buff_info->buff_top, return_code);
			fwrite((char *)sei_stream_buff_info->buff_top, return_code, 1, appli_info->output_file_fp);
	...
	if (appli_info->other_API_enc_param.out_recovery_point_SEI == AVCBE_ON) {
		return_code = avcbe_put_SEI_parameters(context, AVCBE_SEI_MESSAGE_RECOVERY_POINT,
						 (void *)&(appli_info->other_API_enc_param.sei_recovery_point_param), sei_stream_buff_info);
		/* concatenate SEI parameter (return_code means the size of SEI parameter in byte unit) */
		if (return_code > 0) {
			cat_output_stream(appli_info, context, (char *)sei_stream_buff_info->buff_top, return_code);
			fwrite((char *)&sei_stream_buff_info->buff_top,	return_code, 1, appli_info->output_file_fp);
