 ...
 */
typedef struct {
	unsigned char *Y_fmemp; /**< Pointer to the luma frame */
	unsigned char *C_fmemp; /**< Pointer to the chroma frame */
} TAVCBD_FMEM;
 ...
 */
typedef struct {
	long error_num; /**< error ID code */
	unsigned long error_pos; /**< bit position where error was found */
	unsigned long detect_param; /* flags to indicate detection of pic_param, seq_param, etc. */
	unsigned long read_slices; /**< number of slice(s) that's been processed */
	unsigned long read_bits; /**< number of bits that's been processed */
	long last_macroblock_pos; /**< last decoded MB position  */
	unsigned long frame_num; /**< frame_num in slice_header() */
	unsigned long poc_top; /**< POC (PictureOrderCnt) */
	unsigned long poc_bottom; /**< reserved (BottomFieldCnt) */
	unsigned long primary_pic_type; /**< valid if access_unit_delimiter() is present */
...
typedef struct {
	unsigned char pic_width_in_mbs; /**< width */
	unsigned char pic_height_in_map_units; /**< height */
	short pic_width; /**< width in pixel unit */
	short pic_height; /**< height in pixel unit */
	short pic_size_in_mbs; /**< number of mbs in one frame */
} AVCBD_PIC_SIZE;
 ...
 */
typedef struct {
	unsigned char profile_idc; /**< Baseline maybe 66 */
	unsigned char constraint_set_flag; /**< mixture of set[0-2] + 5bit */
	unsigned char level_idc; /**< Level information */
	short seq_parameter_set_id; /**< In the range of [0,31] */
	unsigned short log2_max_frame_num; /**< modified from log2(x)-4 [4,16] */
	unsigned char pic_order_cnt_type; /**< In the range of [0,2] */

  	unsigned short log2_max_pic_order_cnt_lsb; /**< modified from log2(x)-4 [4,16]*/
  	unsigned char delta_pic_order_always_zero_flag; /**< one bit */
  	signed long offset_for_non_ref_pic; /**< signed 32bit value */
  	signed long offset_for_top_to_bottom_field; /**< signed 32bit value */
  	unsigned char num_ref_frames_in_pic_order_cnt_cycle; /**< [0,255] */
  	signed short delta_offset_for_ref_frame[ AVCBD_MAX_POC_CYCLE ]; /**< sum of signed 32bit value */
	unsigned char num_ref_frames; /**< total number of ref frame. [0,16] */
	unsigned char gaps_in_frame_num_value_allowed_flag; /**< one bit */
	AVCBD_PIC_SIZE pic_size;
	unsigned char frame_mbs_only_flag; /**< multiplexed with mb_adaptive_frame_field_flag */
	unsigned char direct_8x8_inferrence_frame_cropping_flag; /**< multiplexed flags */
	struct {
		short left_offset;
		short right_offset;
		short top_offset;
	...
	unsigned char nal_unit_type; /**< nal_unit_type of current NAL */
	unsigned short curr_slice_id; /**< slice # in current picture */
} AVCBD_SEQ_PARAMETER_SET;

/** Picture parameter set.
 */
typedef struct {
	short pic_parameter_set_id; /**< [0,255] */
	unsigned char seq_parameter_set_id; /**< [0,31] */
	unsigned char entropy_coding_mode_flag; /**< one bit */
	unsigned char pic_order_present_flag; /**< one bit */
	unsigned char num_slice_groups; /**< [1,8] in Baselile profile */
	unsigned char slice_group_map_type; /**< [0,6] */
	unsigned char slice_group_change_direction_flag; /**< one bit */
	unsigned short run_length[ 8 ]; /**< [1,maxMBnum] for each elements */
	struct {
		unsigned short top_left; /**< less than bottom_right */
		unsigned short bottom_right; /**< less than maxMBnum */
	} rectangle[ 8 ];
	unsigned short slice_group_change_rate; /**< [1,maxMBnum] */
	unsigned short pic_size_in_map_units; /**< equal to maxMBnum */
	unsigned char slice_group_id[ 8 ]; /**< [0,num_slice_groups-1] for each elements */
	unsigned char num_ref_idx_l0_active; /**< [1,32] */
	unsigned char num_ref_idx_l1_active; /**< [1,32] */
	unsigned char weighted_pred_flag_bipred_idc; /**< shall be zero */
	unsigned char pic_init_qp; /**< [0,31] */
	unsigned char pic_init_qs; /**< [0,31] */
	signed char chroma_qp_index_offset; /**< [-12,12]*/
	unsigned char deblocking_filter_control_present_flag; /**< one bit */
	unsigned char constrained_intra_pred_flag; /**< one bit */
	unsigned char redundant_pic_cnt_present_flag; /**< one bit */
...
typedef long (* TAVCBD_DEC_CONTINUE_FUNC)(const void *context, char **next_stream_buff, unsigned long *stream_buff_size);

unsigned long avcbd_get_version( void );
long avcbd_get_workarea_size(int stream_type, long x, long y, long pic_param_num);
long avcbd_get_context_size( long wx, long wy, long pic_param_num );
long avcbd_start_decoding( void );
long avcbd_init_sequence( void * workarea, long workarea_size, unsigned long nfmem, 
                          TAVCBD_FMEM fmema[],
                          long wx, long wy, long pic_param_num,
                          long *dp1_addr, long *dp2_addr,
                          long stream_type,void **context);
long avcbd_set_stream_pointer( void *context, unsigned char *stream_buff, unsigned long stream_size, TAVCBD_DEC_CONTINUE_FUNC func );
long avcbd_set_decode_mode( void *context, long decode_unit_type );
long avcbd_get_last_frame_stat( void *context, TAVCBD_LAST_FRAME_STATUS *status );
long avcbd_get_frame_size( void *context, TAVCBD_FRAME_SIZE *frame_size );
long avcbd_decode_picture( void *context, long max_read_bits );
long avcbd_get_decoded_frame( void *context, long mode );
long avcbd_set_resume_err( void *context, long err_resume_flg, long err_conceal_mode );

/* temporal scalability */
long avcbd_temporal_init_sequence( void *contextB, void *contextE, long context_size, unsigned long nfmem, 
			  TAVCBD_FMEM fmema[],
			  long wx, long wy,
			  long *dp1_addr, long *dp2_addr );
long avcbd_temporal_set_stream_pointer( void *contextB, void *contextE,
...
long avcbd_temporal_decode_picture( void *contextB, void *contextE, long max_read_bits );
long avcbd_temporal_get_last_frame_stat( void *contextB, void *contextE,
					TAVCBD_LAST_FRAME_STATUS *status );
long avcbd_temporal_get_decoded_frame( void *contextB, void *contextE, long mode );


/* supplimental functions */
long avcbd_search_start_code( unsigned char *, long, unsigned long );
long avcbd_extract_nal( void *pSrc, void *pDst, long iSize, unsigned long iMode );
long avcbd_emulation_prevention_byte_skip( void *context, long on );

void avcbd_get_time_code( long hour, long min, long sec);
long avcbd_search_vop_header( void *context, unsigned char *stream, long search_max );
long avcbd_get_sc_interval( void *context, unsigned char *stream, long search_max, long *header_type );
...
long avcbd_set_filter_mode( void *context, long filter_mode, long filter_select, TAVCBD_FMEM *filtered_fmem );
long avcbd_sc_parse( void *context, long sc_timeout_nbit, long vol_autolock, long vol_parse_only );
