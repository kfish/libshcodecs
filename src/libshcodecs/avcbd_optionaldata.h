...
typedef struct {
	...
	TAVCBD_SEI_PIC_TIMING_TIMESTAMP timestamp[ AVCBD_MAX_CLOCK_TS ];
...
typedef struct {
	...
	TAVCBD_SEI_DEC_REF_PIC_MARKING_REPETITION marking_repetition;
} TAVCBD_SEI;
 ...
 */
typedef long (* TAVCBD_SEI_PAYLOAD_FUNC)( void *context, unsigned long type, unsigned long size, unsigned char *data );
...
long avcbd_init_memory_optional(
	void *context, unsigned long buffer_type, void *buffer, long size );
